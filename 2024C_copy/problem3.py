...
last_season_data = pd.read_excel('./results/2-2029/2029-粮食.xlsx')
...
def func1(x, Y=Y):
    ...
    sum_y = 0
    for k in K1:
        sum_y += Y(x, k, J1, crop_land_to_yield_2024, crop_price_2024)
    sum_jk = 0
    for j in J1:
        for k in K1:
            sum_jk += x[j_transform(j), k_transform(k)] * C(j, k, crop_land_to_cost_2024) * S(j)
    f = sum_y - sum_jk
    ...
    for crops in [crop_staple, crop_grain, crop_tuber]:
        ...
        for k in crops:
            if k != 15:
                sum_land_c += X(x, k, J1, crop_land_to_yield_2024)
                sum_land_r += R(k, crop_sale_2024)
...
# result_to_excel(np.array(res1.get('Vars')).reshape(dim_x, dim_y), J1, K1, './results/3-2024/2024-粮食.xlsx')
...
def func2(x, Y=Y):
    ...
    sum_y = 0
    for k in K2:
        sum_y += Y(x, k, J2, crop_land_to_yield_2024, crop_price_2024)
    sum_jk = 0
    for j in J2:
        for k in K2:
            sum_jk += x[j_transform(j), k_transform(k)] * C(j, k, crop_land_to_cost_2024) * S(j)
    ...
    for crops in [vage_egg, vage_leaf, vage_cabbage, vage_root]:
        ...
        for k in crops:
            if k in K2:
                sum_land_c += X(x, k, J2, crop_land_to_yield_2024)
                sum_land_r += R(k, crop_sale_2024)
        cv.append(sum_land_c - sum_land_r)
...
result_to_excel(np.array(res2.get('Vars')).reshape(dim_x, dim_y), J2, K2, './results/3-2024/2024-蔬菜.xlsx')
...
last_season_data = pd.read_excel('./results/2-2030/2030-蔬菜.xlsx')
...
def func3(x, Y=Y):
    ...
    sum_y = 0
    for k in K3:
        sum_y += Y(x, k, J3, crop_land_to_yield_2024, crop_price_2024)
    sum_jk = 0
    for j in J3:
        for k in K3:
            sum_jk += x[j_transform(j), k_transform(k)] * C(j, k, crop_land_to_cost_2024) * S(j)
    ...
    for crops in [vage_leaf, vage_root]:
        ...
        for k in crops:
            if k in K3:
                sum_land_c += X(x, k, J3, crop_land_to_yield_2024)
                sum_land_r += R(k, crop_sale_2024)
        cv.append(sum_land_c - sum_land_r)
...
# result_to_excel(np.array(res3.get('Vars')).reshape(dim_x, dim_y), J3, K3, './results/3-2024/2024-萝卜-第二季.xlsx')

last_season_data = pd.read_excel('./results/3-2024/2024-蔬菜.xlsx')
...
def func4(x, Y=Y):
    ...
    sum_y = 0
    for k in K4:
        sum_y += Y(x, k, J4, crop_land_to_yield_2024, crop_price_2024)
    sum_jk = 0
    for j in J4:
        for k in K4:
            sum_jk += x[j_transform(j), k_transform(k)] * C(j, k, crop_land_to_cost_2024) * S(j)
    ...
    for crops in [vage_egg, vage_leaf, vage_cabbage, vage_root]:
        ...
        for k in crops:
            if k in K4:
                sum_land_c += X(x, k, J4, crop_land_to_yield_2024)
                sum_land_r += R(k, crop_sale_2024)
        cv.append(sum_land_c - sum_land_r)
...
# result_to_excel(np.array(res4.get('Vars')).reshape(dim_x, dim_y), J4, K4, './results/3-2024/2024-蔬菜-第二季.xlsx')